...
class RecurrentNeuralNetworkTrainer(BaseTrainer):
    def __init__(self, sess, model, flags):
        super(RecurrentNeuralNetworkTrainer, self).__init__(sess, model.config, flags)
        ...
        while self.cur_epoch_tensor.eval(self.sess) < self.model.config.training_epochs:
            ...
            for i in range(total_batch):
                ...
                batch_x = batch_x.reshape(
                    (self.model.config.batch_size, self.model.config.n_steps, self.model.config.n_input))
            ...
            summary_dic = {'loss': avg_cost, 'train_accuracy': accuracy}
    ...
    def test(self):
        batch_x = self.mnist.test.images.reshape(
            (-1, self.model.config.n_steps, self.model.config.n_input))
        accuracy = self.model.accuracy.eval(session=self.sess, feed_dict={self.model.X: batch_x,
                                                                          self.model.y: self.mnist.test.labels,
                                                                          self.model.is_training: False})
        tf.logging.info("Test Accuracy: {:.5f}".format(accuracy))
        summary_dic = {'test_accuracy': accuracy}
        self.add_summary(self.cur_epoch_tensor.eval(self.sess), summary_dic)
